...
if ( ! function_exists( 'wc_create_new_customer' ) ) {
	...
	function wc_create_new_customer( $email, $username = '', $password = '', $args = array() ) {
		...
		//讓註冊頁面可驗證是否為手機格式
		$phone_arr = str_split($username,2); //分割字串，偵測是否為'09'開頭
		if ( strlen($username)!=10 || $phone_arr[0] != "09") {
			return new WP_Error( 'registration-error-invalid-exists', __( '請輸入正確手機格式 09xxxxxxxx', 'woocommerce' ) );
		}
